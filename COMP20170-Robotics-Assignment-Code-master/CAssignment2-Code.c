...
const float ROBOT_MOTOR_RATIO = 0.5;

//Time Constant
const float dt = 0.01;

//Gain Constants
const float PROPORTIONAL_GAIN = 50;
const float INTEGRAL_GAIN = 140;
const float DERIVATE_GAIN = 0.3;
...
task main()
{
  ...
  displayCenteredTextLine(7,"PID = %f",pid);
  displayCenteredTextLine(9,"Angle = %f,", robotAngle);
  ...
  displayCenteredTextLine(11,"Motor = %ld", motorVelocity);
  	...
  	pid = (PROPORTIONAL_GAIN * error + INTEGRAL_GAIN * integralError + DERIVATE_GAIN * errorChange)/WHEEL_DIAMETER;
